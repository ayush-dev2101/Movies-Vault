...
const WatchlistScreen = ({ navigation }: any) => {
  ...
  const loadWatchlist = async () => {
    try {
      ...
      if (status === 401) {
        // Safe to ignore: User backend sync is likely still in progress
        setMovies([]);
      } else if (error.message === 'Server timeout. Please try again.') {
        Alert.alert(
          'Server Waking Up',
          'The backend is waking up from sleep. Please try refreshing in a few seconds.'
        );
      } else {
  ...
  const renderItem = ({ item }: { item: any }) => (
    ...
    >
      ...
      <View style={styles.info}>
        ...
        <TouchableOpacity
          onPress={() => removeItem(item.movieId)}
