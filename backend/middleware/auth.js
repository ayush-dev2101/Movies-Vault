...
const getJwksKeys = async (issuer) => {
  ...
  console.log(
    `[Backend-Trace] Fetching fresh JWKS keys from issuer: ${issuer}`,
  );
  const response = await fetch(`${issuer}/.well-known/jwks.json`);
  if (!response.ok) {
    throw new Error(
      `Failed to fetch JWKS keys from issuer: ${response.statusText}`,
    );
  }
...
const protect = async (req, res, next) => {
  ...
  if (
    ...
    try {
      ...
      console.log(
        `[Backend-Trace] Auth Middleware - Received Token (Length: ${token.length})`,
      );
      ...
      if (!decodedToken || !decodedToken.header || !decodedToken.payload) {
        console.error(
          "[Backend-Trace] Auth Middleware - Invalid or malformed JWT structure",
        );
      ...
      console.log(
        "[Backend-Trace] Decoded JWT Header:",
        JSON.stringify(decodedToken.header),
      );
      console.log(
        "[Backend-Trace] Decoded JWT Payload (truncated sub/iss):",
        JSON.stringify({
      ...
      if (!iss) {
        console.error(
          "[Backend-Trace] Auth Middleware - Missing issuer (iss) claim",
        );
        return res.status(401).json({
      ...
      if (!iss.includes("clerk") && !iss.includes("accounts.dev")) {
        console.error(
          `[Backend-Trace] Auth Middleware - Untrusted Issuer: ${iss}`,
        );
        return res.status(401).json({
      ...
      try {
        ...
        console.error(
          `[Backend-Trace] Auth Middleware - Failed to fetch JWKS keys from ${iss}:`,
          fetchError.message,
        );
        return res.status(401).json({
      ...
      if (!jwk) {
        console.error(
          `[Backend-Trace] Auth Middleware - Matching JWK with kid ${kid} not found in JWKS`,
        );
        return res.status(401).json({
      ...
      try {
        ...
        console.error(
          "[Backend-Trace] Auth Middleware - Native Crypto JWK key conversion failed:",
          cryptoError.message,
        );
        return res.status(401).json({
      ...
      jwt.verify(
        ...
        async (err, verified) => {
          if (err) {
            console.error(
              `[Backend-Trace] Auth Middleware - Signature verification failed: ${err.message}`,
            );
            return res.status(401).json({
              message:
                "Not authorized, cryptographic token verification failed",
              error: err.message,
          ...
          console.log(
            `[Backend-Trace] Auth Middleware - Cryptographically verified Clerk JWT for: ${clerkId}`,
          );
          ...
          if (!user) {
            console.warn(
              `[Backend-Trace] Auth Middleware - Verified user ${clerkId} not found in DB yet.`,
            );
      ...
      console.error(
        "[Backend-Trace] Cryptographic Auth Middleware Error:",
        error.message,
      );
      return res
        .status(401)
